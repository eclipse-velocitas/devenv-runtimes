...
def test_resolve_functions__pathInWorkspaceOrPackage__file_not_in_workspace_or_package__raises_exception():
    with pytest.raises(RuntimeError, match="Path 'asd' not found in workspace or package!"):
...
def test_resolve_functions__pathInWorkspaceOrPackage__file_exists_in_workspace():
    assert resolve_functions('$pathInWorkspaceOrPackage( manifest.json )') == os.path.join(get_workspace_dir(), 'manifest.json')


def test_resolve_functions__pathInWorkspaceOrPackage__file_exists_in_workspace():
    os.environ['VELOCITAS_WORKSPACE_DIR'] = '..'
    assert resolve_functions('$pathInWorkspaceOrPackage( manifest.json )') == os.path.join(get_package_path(), 'manifest.json')
